...
vim.opt.shortmess:append 'sI' -- remove nvim intro
vim.o.backup = false
...
require('lazy').setup({
    ...
    config = function()
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          map('<leader>s', vim.lsp.buf.rename, '[R]e[n]ame')
          ...
          -- Replace this section in your LSP config (around line 80-120)
          ...
          if client and client_supports_method(client, vim.lsp.protocol.Methods.textDocument_documentHighlight, event.buf) then
            ...
            vim.api.nvim_create_autocmd({ 'CursorMoved', 'CursorMovedI' }, {
              ...
              group = highlight_augroup,
              callback = function()
                -- Check if buffer is still valid before clearing references
                if vim.api.nvim_buf_is_valid(event.buf) then
            ...
            vim.api.nvim_create_autocmd('LspDetach', {
              buffer = event.buf, -- Important: attach to specific buffer
              group = vim.api.nvim_create_augroup('kickstart-lsp-detach-' .. event.buf, { clear = true }),
              callback = function(event2)
                -- Clear references and autocmds when LSP detaches
                ...
                -- Clean up the detach augroup too
                vim.api.nvim_del_augroup_by_name('kickstart-lsp-detach-' .. event2.buf)
              end,
            })
          end
  ...
  },
  {
    'mbbill/undotree',
    keys = {
      { '<C-s>', '<cmd>UndotreeToggle<cr>', desc = 'toggle the undoree' },
    },
  },
